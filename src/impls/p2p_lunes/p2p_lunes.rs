...
pub trait P2pLunesImpl: Storage<Data> +
    ...
    Storage<ownable::Data>
{
       ...
       fn cancel_order(&mut self, id: u64) -> Result<(), PSP22Error> {
           let caller = Self::env().caller();
           let index = self.data::<Data>().books
               .iter()
       ...
       fn buy_order(&mut self, id: u64, quantity: Balance) -> Result<(), PSP22Error> {
           let index =  self.data::<Data>().books.iter().position(|order| order.id == id && order.cencel == false);
           if index.is_none() {
       ...
       fn confirm_sell(&mut self, id: u64) -> Result<(), PSP22Error> {
           let caller = Self::env().caller();
           let index = self.data::<Data>().buy_books
               .iter()
               .position(
       ...
       /// Confirm conflit with sale
       #[ink(message)]
       #[modifiers(non_reentrant)]
       #[openbrush::modifiers(only_owner)]
       fn conflict_sele(&mut self, id: u64, confirm_payment: bool) -> Result<(), PSP22Error> {
           let date_block = Self::env().block_timestamp();
           let index = self.data::<Data>().buy_books
               .iter()
               .position(
                   |order|
                       order.id == id && order.conflict == true && order.date_expire > date_block
               );
           if index.is_none() {
            return Err(PSP22Error::Custom(LunesError::NoBuyBook.as_str()));
           }
       ...
       fn open_conflict_user(&mut self, id: u64) -> Result<(), PSP22Error> {
           ...
           let index =  self.data::<Data>().buy_books
               .iter()
               .position(
                   |order| order.id == id && order.owner == caller && order.confirmed == false
               );
           if index.is_none() {
            return Err(PSP22Error::Custom(LunesError::NoBuyBook.as_str()));
           }
       ...
       fn buy_books_seller(&mut self, page: u64) -> Result<Vec<BuyBook>, PSP22Error> {
           ...
           _all_buy =  self.data::<Data>().buy_books
               .iter()
       ...
       fn close_buy_user(&mut self, id:u64) -> Result<(), PSP22Error> {
           ...
           let index =  self.data::<Data>().buy_books
               .iter()
               .position(
                   |order| order.id == id && order.owner == caller && order.confirmed == true
               );
           if index.is_none() {
            return Err(PSP22Error::Custom(LunesError::NoBuyBook.as_str()));
           }
